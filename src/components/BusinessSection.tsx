...
export const BusinessSection: React.FC = () => {
  ...
  const [endorsementModalBusiness, setEndorsementModalBusiness] = useState<BusinessListing | null>(null);
  const [endorsementComment, setEndorsementComment] = useState('');
  const [endorsementFilter, setEndorsementFilter] = useState<'All' | 'Reliability' | 'Ethics' | 'Service Quality'>('All');
  ...
  const filtered = businesses.filter((b) => {
    ...
    if (endorsementFilter !== 'All') {
      const hasCat = (b.endorsements || []).some((e) => e.category === endorsementFilter);
  ...
  const handleDownloadVisitingCardJpg = async () => {
    ...
    try {
      ...
      const canvas = await safeHtml2Canvas(node, {
        scale: 2,
        useCORS: true,
        allowTaint: true,
  ...
  const handleDownloadVisitingCardPdf = async () => {
    const node = document.getElementById('visiting-card-studio-node');
    if (!node) return;
    try {
      showToast('Generating PDF', 'Converting visiting card to formatted PDF...', 'info');
      const canvas = await safeHtml2Canvas(node, {
        scale: 3,
        useCORS: true,
        allowTaint: true,
      ...
      const filename = `Visiting_Card_${(visitingCardBusiness?.businessName || 'Business').replace(/[^a-zA-Z0-9]/g, '_')}.pdf`;
      pdf.save(filename);
      showToast('Visiting Card PDF Downloaded! 📄', 'Saved as formatted PDF visiting card.', 'success');
    } catch (err) {
      console.error('Error generating PDF visiting card:', err);
      showToast('PDF Download Error', 'Unable to render card into PDF document.', 'error');
    }
  };
...
END:VCARD`;
  ...
  const handleDownloadQrPosterPdf = async () => {
    const node = document.getElementById('business-qr-poster-node');
    if (!node || !qrModalBusiness) return;
    try {
      showToast('Generating PDF Poster', 'Formatting high-res QR standee PDF document...', 'info');
      const canvas = await safeHtml2Canvas(node, {
        scale: 3,
        useCORS: true,
        allowTaint: true,
      ...
      // Calculate width/height to center on A4 page (210mm x 297mm)
      const pdfWidth = 190; // 10mm margins on sides
      const imgProps = pdf.getImageProperties(imgData);
      const pdfHeight = (imgProps.height * pdfWidth) / imgProps.width;

      pdf.addImage(imgData, 'JPEG', 10, 15, pdfWidth, pdfHeight);
      
      const filename = `Business_QR_Poster_${qrModalBusiness.businessName.replace(/[^a-zA-Z0-9]/g, '_')}.pdf`;
      pdf.save(filename);
      showToast('QR Poster PDF Downloaded! 📄', 'Saved as printable A4 PDF poster.', 'success');
    } catch (err) {
      console.error('Error generating QR poster PDF:', err);
      showToast('Poster PDF Failed', 'Unable to render QR poster into PDF format.', 'error');
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-3">
        ...
        {/* Category Chips & Count Banner */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-t border-slate-100 dark:border-slate-800 pt-2 text-xs font-bold text-slate-600 dark:text-slate-300">
          <div className="flex items-center gap-2">
            <span>Showing <strong>{filtered.length}</strong> of <strong>{businesses.length}</strong> Total Database Listings</span>
            {(selectedCategory !== 'All' || searchTerm || onlyGstVerified) && (
              <button
                onClick={() => {
                  setSelectedCategory('All');
                  setSearchTerm('');
                  setOnlyGstVerified(false);
                }}
              ...
              >
                Clear Filters (Show All {businesses.length})
              </button>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 pt-2 border-t border-slate-100 dark:border-slate-800 overflow-x-auto no-scrollbar">
          <span className="text-[11px] font-extrabold uppercase text-slate-400 shrink-0 flex items-center gap-1">
            ...
            <span>Filter Endorsed:</span>
          </span>
          
          <button
            onClick={() => setEndorsementFilter('All')}
          ...
          <button
            onClick={() => setEndorsementFilter('Reliability')}
            className={`px-3 py-1.5 rounded-xl text-xs font-bold whitespace-nowrap transition-all flex items-center gap-1 cursor-pointer ${
              endorsementFilter === 'Reliability'
                ? 'bg-blue-600 text-white shadow'
                : 'bg-blue-50 dark:bg-blue-950/60 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800/80 hover:bg-blue-100'
            }`}
          >
            <ShieldCheck className="w-3.5 h-3.5 text-blue-500" />
            <span>Reliability Endorsed</span>
          </button>

          <button
            onClick={() => setEndorsementFilter('Ethics')}
            className={`px-3 py-1.5 rounded-xl text-xs font-bold whitespace-nowrap transition-all flex items-center gap-1 cursor-pointer ${
              endorsementFilter === 'Ethics'
          ...
          >
            ...
            <span>Ethics Endorsed</span>
          </button>

          <button
            onClick={() => setEndorsementFilter('Service Quality')}
            className={`px-3 py-1.5 rounded-xl text-xs font-bold whitespace-nowrap transition-all flex items-center gap-1 cursor-pointer ${
              endorsementFilter === 'Service Quality'
                ? 'bg-amber-600 text-white shadow'
          ...
          >
            <Award className="w-3.5 h-3.5 text-amber-500" />
            <span>Service Quality Endorsed</span>
          </button>
        </div>
      </div>
                  ...
                  <div className="pt-2 border-t border-slate-100 dark:border-slate-800 space-y-1.5">
                    ...
                    <div className="flex flex-wrap items-center gap-1.5">
                      ...
                      <button
                        ...
                        className={`px-2 py-0.5 rounded-full text-[10px] font-extrabold border flex items-center gap-1 cursor-pointer transition-all ${
                          (b.endorsements || []).some((e) => e.category === 'Reliability')
                        ...
                        title="Reliability Endorsements"
                      >
                        <ShieldCheck className="w-3 h-3 text-blue-500" />
                        <span>Reliability ({(b.endorsements || []).filter((e) => e.category === 'Reliability').length})</span>
                      </button>
                      ...
                      <button
                        ...
                        className={`px-2 py-0.5 rounded-full text-[10px] font-extrabold border flex items-center gap-1 cursor-pointer transition-all ${
                          (b.endorsements || []).some((e) => e.category === 'Ethics')
                        ...
                        title="Ethics Endorsements"
                      >
                        <Sparkles className="w-3 h-3 text-emerald-500" />
                        <span>Ethics ({(b.endorsements || []).filter((e) => e.category === 'Ethics').length})</span>
                      </button>
                      ...
                      <button
                        ...
                        className={`px-2 py-0.5 rounded-full text-[10px] font-extrabold border flex items-center gap-1 cursor-pointer transition-all ${
                          (b.endorsements || []).some((e) => e.category === 'Service Quality')
                        ...
                        title="Service Quality Endorsements"
                      >
                        <Award className="w-3 h-3 text-amber-500" />
                        <span>Quality ({(b.endorsements || []).filter((e) => e.category === 'Service Quality').length})</span>
                      </button>
                    </div>
                  </div>
                </div>
        ...
        return (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-md overflow-y-auto">
            <div className="bg-white dark:bg-slate-900 border border-amber-500/50 rounded-3xl w-full max-w-lg p-6 shadow-2xl relative text-slate-800 dark:text-slate-100 space-y-5 my-auto">
              ...
              {/* CONTROLS: REGENERATE DESIGN, QR POSTER, DOWNLOAD PDF & JPG */}
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-2">
                <button
                  onClick={() => setCardDesignIndex((prev) => (prev + 1) % CARD_DESIGN_STYLES.length)}
                  className="py-2.5 px-2 min-h-[44px] bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-100 font-extrabold text-xs rounded-2xl shadow border border-slate-300 dark:border-slate-700 flex items-center justify-center gap-1.5 transition-all cursor-pointer"
                >
                ...
                <button
                  ...
                  className="py-2.5 px-2 min-h-[44px] bg-amber-500/10 hover:bg-amber-500/20 text-amber-700 dark:text-amber-300 font-extrabold text-xs rounded-2xl shadow border border-amber-500/30 flex items-center justify-center gap-1.5 transition-all cursor-pointer"
                >
                  <Printer className="w-3.5 h-3.5 text-amber-500" />
                  <span>QR Poster</span>
                </button>
                ...
                <button
                  onClick={handleDownloadVisitingCardPdf}
                  className="py-2.5 px-2 min-h-[44px] bg-gradient-to-r from-amber-500 to-amber-700 text-amber-950 font-extrabold text-xs rounded-2xl shadow-lg hover:from-amber-600 hover:to-amber-800 flex items-center justify-center gap-1.5 transition-all cursor-pointer"
                >
                ...
                <button
                  ...
                  className="py-2.5 px-2 min-h-[44px] bg-slate-900 text-amber-300 dark:bg-amber-500 dark:text-slate-950 font-extrabold text-xs rounded-2xl shadow hover:opacity-90 flex items-center justify-center gap-1.5 transition-all cursor-pointer"
                >
      ...
      {/* COMMUNITY ENDORSEMENT MODAL */}
      {endorsementModalBusiness && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-fade-in">
          <div className="bg-white dark:bg-slate-900 border-2 border-amber-500/40 rounded-3xl max-w-2xl w-full p-5 sm:p-7 shadow-2xl space-y-6 max-h-[90vh] overflow-y-auto relative text-slate-900 dark:text-slate-100">
            
            {/* Header */}
            <div className="flex items-start justify-between gap-3 pb-4 border-b border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-3">
                <img
                  src={endorsementModalBusiness.logoUrl}
                  alt={endorsementModalBusiness.businessName}
                  className="w-14 h-14 rounded-2xl object-cover border-2 border-amber-400 shrink-0 bg-white"
                />
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-black uppercase tracking-wider text-amber-600 dark:text-amber-400">
                      Community Endorsement & Trust Badges
                    </span>
                  </div>
                  <h3 className="text-lg font-extrabold text-slate-900 dark:text-white">
                    {endorsementModalBusiness.businessName}
                  </h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    {endorsementModalBusiness.city}, {endorsementModalBusiness.state} • {endorsementModalBusiness.category}
                  </p>
                </div>
              </div>
              ...
              <button
                onClick={() => {
                  setEndorsementModalBusiness(null);
                  setEndorsementComment('');
                }}
            ...
            {/* Info Notice */}
            <div className="p-4 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-900/60 rounded-2xl text-xs text-amber-900 dark:text-amber-200 space-y-1">
              <p className="font-extrabold flex items-center gap-1.5">
                <ShieldCheck className="w-4 h-4 text-amber-600" />
                <span>Jain Community Peer Vouching System</span>
              </p>
              <p className="text-[11px] opacity-90 leading-relaxed">
                Authenticated members can endorse businesses for <strong>Reliability</strong>, <strong>Ethics</strong>, or <strong>Service Quality</strong>.
                Endorsements enhance business visibility and display verified community trust badges on the business card.
              </p>
            </div>

            {/* Endorsement Categories Action Cards */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {/* Category 1: Reliability */}
              {(() => {
                const categoryEndorsements = (endorsementModalBusiness.endorsements || []).filter(e => e.category === 'Reliability');
                const isEndorsedByMe = currentUser && categoryEndorsements.some(e => e.userId === currentUser.id);
                  ...
                  <div className={`p-4 rounded-2xl border-2 transition-all flex flex-col justify-between space-y-3 ${
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="px-2 py-0.5 bg-blue-100 dark:bg-blue-900/80 text-blue-800 dark:text-blue-200 font-extrabold text-[11px] rounded-full">
                          {categoryEndorsements.length} Endorsed
                        </span>
                      </div>
                      <h4 className="font-extrabold text-sm text-slate-900 dark:text-white pt-1">Reliability</h4>
                      <p className="text-[11px] text-slate-500 dark:text-slate-400 leading-tight">
                        Timely deliveries, contractual commitments & dependable fulfillment.
                      </p>
                    </div>
                    ...
                    <button
                      onClick={() => {
                        if (!currentUser) {
                          showToast('Sign In Required', 'Please sign in or register to endorse businesses.', 'info');
                          setIsAuthModalOpen(true);
                          return;
                        }
                        endorseBusiness(endorsementModalBusiness.id, 'Reliability', endorsementComment);
                        const updatedEndorsements = isEndorsedByMe
                          ? (endorsementModalBusiness.endorsements || []).filter(e => !(e.userId === currentUser.id && e.category === 'Reliability'))
                          : [
                              {
                                id: `end_${Date.now()}`,
                              ...
                              ...(endorsementModalBusiness.endorsements || []),
                            ];
                        setEndorsementModalBusiness({
                          ...endorsementModalBusiness,
                      ...
                      className={`w-full py-2 px-3 rounded-xl font-extrabold text-xs transition-all flex items-center justify-center gap-1.5 cursor-pointer ${
                        isEndorsedByMe
                          ...
                          : 'bg-white dark:bg-slate-900 text-blue-600 border border-blue-300 dark:border-blue-700 hover:bg-blue-50'
                      }`}
                    >
                      {isEndorsedByMe ? (
                        <>
              ...
              {/* Category 2: Ethics */}
              {(() => {
                const categoryEndorsements = (endorsementModalBusiness.endorsements || []).filter(e => e.category === 'Ethics');
                const isEndorsedByMe = currentUser && categoryEndorsements.some(e => e.userId === currentUser.id);
                  ...
                  <div className={`p-4 rounded-2xl border-2 transition-all flex flex-col justify-between space-y-3 ${
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="px-2 py-0.5 bg-emerald-100 dark:bg-emerald-900/80 text-emerald-800 dark:text-emerald-200 font-extrabold text-[11px] rounded-full">
                          {categoryEndorsements.length} Endorsed
                        </span>
                      </div>
                      <h4 className="font-extrabold text-sm text-slate-900 dark:text-white pt-1">Ethics</h4>
                      <p className="text-[11px] text-slate-500 dark:text-slate-400 leading-tight">
                        Transparent dealings, Sattvic/Ahimsa standards & fair pricing.
                      </p>
                    </div>
                    ...
                    <button
                      onClick={() => {
                        ...
                        endorseBusiness(endorsementModalBusiness.id, 'Ethics', endorsementComment);
                        const updatedEndorsements = isEndorsedByMe
                          ? (endorsementModalBusiness.endorsements || []).filter(e => !(e.userId === currentUser.id && e.category === 'Ethics'))
                          : [
                              {
                                id: `end_${Date.now()}`,
                                userId: currentUser.id,
                      ...
                      className={`w-full py-2 px-3 rounded-xl font-extrabold text-xs transition-all flex items-center justify-center gap-1.5 cursor-pointer ${
                        isEndorsedByMe
                          ...
                          : 'bg-white dark:bg-slate-900 text-emerald-600 border border-emerald-300 dark:border-emerald-700 hover:bg-emerald-50'
                      }`}
                    >
                      {isEndorsedByMe ? (
                        <>
              ...
              {(() => {
                const categoryEndorsements = (endorsementModalBusiness.endorsements || []).filter(e => e.category === 'Service Quality');
                const isEndorsedByMe = currentUser && categoryEndorsements.some(e => e.userId === currentUser.id);

                return (
                  <div className={`p-4 rounded-2xl border-2 transition-all flex flex-col justify-between space-y-3 ${
                    isEndorsedByMe
                      ? 'bg-amber-50/80 dark:bg-amber-950/60 border-amber-500 shadow-sm'
                      : 'bg-slate-50 dark:bg-slate-800/60 border-slate-200 dark:border-slate-700'
                  }`}>
                    <div className="space-y-1">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="px-2 py-0.5 bg-amber-100 dark:bg-amber-900/80 text-amber-800 dark:text-amber-200 font-extrabold text-[11px] rounded-full">
                          {categoryEndorsements.length} Endorsed
                        </span>
                      </div>
                      <h4 className="font-extrabold text-sm text-slate-900 dark:text-white pt-1">Service Quality</h4>
                      <p className="text-[11px] text-slate-500 dark:text-slate-400 leading-tight">
                        Superior craftsmanship, polite support & dedicated care.
                      </p>
                    </div>
                    ...
                    <button
                      onClick={() => {
                        ...
                        endorseBusiness(endorsementModalBusiness.id, 'Service Quality', endorsementComment);
                        const updatedEndorsements = isEndorsedByMe
                          ? (endorsementModalBusiness.endorsements || []).filter(e => !(e.userId === currentUser.id && e.category === 'Service Quality'))
                          : [
                              {
                                id: `end_${Date.now()}`,
                                userId: currentUser.id,
                      ...
                      className={`w-full py-2 px-3 rounded-xl font-extrabold text-xs transition-all flex items-center justify-center gap-1.5 cursor-pointer ${
                        isEndorsedByMe
                          ...
                          : 'bg-white dark:bg-slate-900 text-amber-600 border border-amber-300 dark:border-amber-700 hover:bg-amber-50'
                      }`}
                    >
                      {isEndorsedByMe ? (
                        <>
            ...
            <div className="space-y-1.5 pt-2">
              <label className="text-xs font-bold text-slate-700 dark:text-slate-300 flex items-center gap-1">
                ...
                <span>Optional Community Testimonial Note (Applies when endorsing)</span>
              </label>
              <input
                type="text"
                value={endorsementComment}
                onChange={(e) => setEndorsementComment(e.target.value)}
                placeholder="e.g., Transparent pricing and trusted family business for years..."
                className="w-full px-3.5 py-2.5 text-xs rounded-xl bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100 border border-slate-200 dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-amber-500"
              />
            </div>
            ...
            <div className="space-y-3 pt-3 border-t border-slate-100 dark:border-slate-800">
              <h4 className="text-xs font-black uppercase tracking-wider text-slate-800 dark:text-slate-200 flex items-center justify-between">
                <span>Community Endorsement Activity ({(endorsementModalBusiness.endorsements || []).length})</span>
                <span className="text-[10px] text-slate-400 font-normal">Verified Jain Member Vouches</span>
              </h4>

              {(endorsementModalBusiness.endorsements || []).length === 0 ? (
                <div className="p-6 text-center text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/40 rounded-2xl border border-dashed border-slate-200 dark:border-slate-700">
                  <Award className="w-8 h-8 text-amber-500/50 mx-auto mb-2" />
                  <p className="text-xs font-bold text-slate-800 dark:text-slate-200">No Community Endorsements Yet.</p>
                  <p className="text-[11px]">Be the first authenticated member to endorse this business above!</p>
                </div>
              ) : (
                <div className="space-y-2.5 max-h-56 overflow-y-auto pr-1">
                  {(endorsementModalBusiness.endorsements || []).map((end) => (
                    <div
                      key={end.id}
                      className="p-3 bg-slate-50 dark:bg-slate-800/60 rounded-2xl border border-slate-200 dark:border-slate-700 flex items-start gap-3"
                    >
                      <img
                        src={end.userPhotoUrl || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&w=100&q=80'}
                        alt={end.userName}
                      ...
                      <div className="flex-1 min-w-0 space-y-1">
                        ...
                        <div className="flex items-center gap-1.5">
                          {end.category === 'Reliability' && (
                            <span className="px-2 py-0.5 bg-blue-100 dark:bg-blue-950 text-blue-800 dark:text-blue-300 border border-blue-300 dark:border-blue-800 text-[10px] font-bold rounded-full flex items-center gap-1">
                              <ShieldCheck className="w-3 h-3 text-blue-500" />
                              <span>Reliability</span>
                            </span>
                          )}
                          {end.category === 'Ethics' && (
                            <span className="px-2 py-0.5 bg-emerald-100 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-800 text-[10px] font-bold rounded-full flex items-center gap-1">
                              <Sparkles className="w-3 h-3 text-emerald-500" />
                              <span>Ethics</span>
                            </span>
                          )}
                          {end.category === 'Service Quality' && (
                            <span className="px-2 py-0.5 bg-amber-100 dark:bg-amber-950 text-amber-800 dark:text-amber-300 border border-amber-300 dark:border-amber-800 text-[10px] font-bold rounded-full flex items-center gap-1">
                              <Award className="w-3 h-3 text-amber-500" />
