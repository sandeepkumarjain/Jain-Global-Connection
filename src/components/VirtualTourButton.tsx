...
import { Compass, Sparkles, Eye, Clock, Image as ImageIcon } from 'lucide-react';
...
export const VirtualTourButton: React.FC<VirtualTourButtonProps> = ({
  ...
  const isAvailable = Boolean(temple.is360Available) || variant === 'carousel';
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    if (isAvailable) {
      ...
      if (showToast) {
        showToast(
          'Virtual Tour Notice',
          `Interactive tour for ${temple.templeName} is launching in the next Tirth digitization release.`,
          'info'
        );
      }
  ...
  if (isAvailable) {
    return (
      <button
        ...
        title="Open interactive Virtual Tour & Holy Site Carousel"
      >
  ...
  return (
    <div className="relative inline-block" onMouseLeave={() => setShowComingSoonTooltip(false)}>
      <button
        ...
        title="Virtual Tour Under Creation — Coming Soon"
      >
